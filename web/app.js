...
(() => {
  ...
  async function overview() {
    ...
    main.innerHTML = `
      ...
      <h2>体检</h2>
      <div class="card"><div class="row"><button id="doctor">跑一次体检（零成本，约 20 秒）</button><span class="muted">scripts/doctor.ts --no-infer：依赖、配置、连通性，不输出凭据值</span></div><pre id="doctorOut" hidden></pre></div>`;
  ...
  async function tickets() {
    ...
    main.innerHTML = `<h1>任务</h1>` + (rows.length ? [...groups.entries()].map(([p, list]) => `
      <h2>${esc(p)}</h2><div class="wrap"><table><thead><tr><th>工单</th><th>阶段</th><th>状态</th><th class="num">成本</th><th>最近</th><th>时间</th></tr></thead><tbody>
      ${list.slice().reverse().map((r) => `<tr><td><a href="#/tickets/${esc(r.ticket)}">${esc(r.ticket)}</a></td><td>${esc(r.stage)}</td><td>${pill(r.state, stateClass[r.state])}</td>
        <td class="num">${money(r.cost)}</td><td>${esc((r.waiting || '').slice(0, 80))}</td><td class="muted">${r.lastAt ? fmtAgo(Date.now() - Date.parse(r.lastAt)) : ''}</td></tr>`).join('')}
      </tbody></table></div>`).join('') : '<p class="muted">暂无工单</p>');
  }
  ...
  async function ticket(id) {
    ...
    const state = d.running ? '在跑' : st?.haltedReason ? '挂起' : closed ? '闭环' : d.paused ? '已暂停' : '等人工';
    const projects = await api('/api/projects');
    const proj = projects.projects.find((p) => p.alias === st?.project);
    main.innerHTML = `
      ...
      <div class="tiles">
        ...
        <div class="tile"><div class="k">游标</div><div class="v">${esc(st?.cursor || '?')}</div><div class="s">${st?.pendingReverify ? `修完回 ${st.pendingReverify}` : ''}</div></div>
        <div class="tile"><div class="k">分支</div><div class="v mono" style="font-size:13px">${esc(st?.branch || '—')}</div><div class="s">${st?.isWorktree ? 'worktree' : ''}</div></div>
        <div class="tile"><div class="k">回环</div><div class="v">${st ? `${st.reviewFixRounds}/${st.acceptanceFixRounds}` : '–'}</div><div class="s">评审 / 验收修复轮</div></div>
      </div>
      ${st?.haltedReason ? `<div class="notice bad">挂起：${esc(st.haltedReason)}</div>` : ''}
      ${st?.pendingGate ? `<div class="notice">待答卡点 ${esc(st.pendingGate.gate)}：${esc(st.pendingGate.summary)}</div>` : ''}
      ${d.pending.length ? `<div class="notice">等回答：${esc(d.pending.join('、'))}</div>` : ''}
      ${closed ? '' : `<div class="actions">
        <button id="pause">暂停（阶段边界停住）</button>
        <button id="resume" class="primary">继续 / 恢复</button>
        <span class="muted">继续会由 daemon 接手并在群里回话；卡点卡原样重发，问题卡重新提问。</span>
      </div>`}
      <h2>各阶段会话</h2>
      ...
      <h2>文档</h2>
      <p>${d.docs.length ? d.docs.map((f) => f.startsWith('prototype/') ? `<span class="pill">${esc(f)}（在「文档」页看）</span>` : `<a href="#/docs/${esc(st.project)}/${esc(id)}/${esc(f)}">${esc(f)}</a>`).join(' · ') : '<span class="muted">无工件目录</span>'}</p>
      <h2>时间线（${d.events.length}）</h2>
      <ul class="timeline">${d.events.slice().reverse().map((e) => `<li><span class="ts">${fmtTs(e.ts).slice(5)}</span><span>${pill(e.type)} ${esc(e.summary)}</span></li>`).join('')}</ul>`;
    void proj;
    if (closed) return;
    ...
    $('#resume').onclick = async () => { try { const r = await post(`/api/tickets/${encodeURIComponent(id)}/resume`); toast(r.note); } catch (e) { toast(e.message, true); } };
  }
  ...
  async function docs(alias, ticket, file) {
    ...
    const ticketFiles = pr && ticket ? await api(`/api/docs/${encodeURIComponent(alias)}/tickets/${encodeURIComponent(ticket)}`) : [];
    const items = pr ? [
    ...
    main.innerHTML = `<h1>文档</h1>
      ...
      <div class="two"><ul class="list" id="tree">${items.map((i) => i.h ? `<li class="h">${i.h}</li>` : i.t
        ? `<li data-t="${esc(i.t)}" class="${i.t === ticket ? 'on' : ''}">${esc(i.label)}</li>` + (i.t === ticket ? ticketFiles.map((f) => `<li data-f="${esc(f)}" class="${f === file ? 'on' : ''}" style="padding-left:28px">${esc(f)}</li>`).join('') : '')
  ...
  const PFIELDS = [['alias', '别名', '字母开头，字母数字-'], ['prefix', '工单号前缀', '1-6 个字母'], ['repo', '本地仓库路径', '正斜杠'], ['gitlab', 'GitLab 路径', 'group/name'], ['jenkins', 'Jenkins 任务', 'folder/job'],
    ['wikiArchive', 'Wiki 归档节点', ''], ['wikiKnowledge', 'Wiki 系统地图节点', ''], ['chatId', '绑定群 chat_id', 'oc_…'], ['owner', '负责人 open_id', 'ou_…（排期卡只认他）']];
  async function projects() {
    const v = await api('/api/projects');
    ...
    const render = () => {
      main.innerHTML = `<h1>项目配置</h1><p class="muted">保存 = 备份 .env、改写 PIPELINE_PROJECTS 一行、daemon 10 秒内原地替换项目表（与群里 /bind、/addproject 同一条路）。</p>
        <form id="pf">${list.map((p, i) => `<div class="card"><div class="form">${PFIELDS.map(([k, label, hint]) => `<label>${esc(label)}</label><input class="mono" data-i="${i}" data-k="${k}" value="${esc(p[k] || '')}" placeholder="${esc(hint)}">`).join('')}</div>
