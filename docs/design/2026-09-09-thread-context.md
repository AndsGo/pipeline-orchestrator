...
**状态**：草稿，待拍板 · **日期**：2026-09-09 · **范围**：编排器（`src/feishu/`、`src/daemon*`、`src/followup.ts`），不动阶段 skill 与 handoff 契约
...
1. 工单话题**默认开**还是按 `PIPELINE.md` 开关？（建议默认开，`audience: business` 的群尤其需要）
2. 广播消息要不要在工单话题里**复制一份**？（建议要，一行文本，让只盯话题的人不漏收尾）
3. 会话寿命 30 轮 / 7 天这两个数。
