...
状态：已实现（2026-09-25）。**后续变更**：同日控制台与 webhook 合并为一个 web 服务（`src/web/`，:8377），决策 1 里的「独立进程」与决策 5 里的「新开 :8378」由此取代——两个服务流量都很小，分开只多了一个端口和一套启动 / 守护。目标：一个网页，管环境配置、项目配置，能热更新 / 热重启，能看任务、需求、文档。
